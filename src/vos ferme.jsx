...
function VosFermes() {
  ...
  const generateFarmSummary = (farmId, weather) => {
    ...
    Object.keys(diseaseRiskCalculators).forEach((disease) => {
        const risk = diseaseRiskCalculators[disease](temperature, humidity, dissolvedOxygen, isOutside);
        const recommendations = getRecommendation(disease, risk, humidity, dissolvedOxygen, language);
    ...
    setSummaries(prevSummaries => ({ ...prevSummaries, [farmId]: Array.from(uniqueRecommendations) }));
};

  useEffect(() => {
    fetch(`https://vite-project-9cea.onrender.com/api/v1/users/${userId}/farms`, {
    ...
    .then(data => {
        ...
        data.forEach(farm => {
            if (farm.address) {
                getWeather(farm.address).then(weather => {
                    console.log(`Weather data for ${farm.name}:`, weather);
                    setWeatherData(prevData => ({ ...prevData, [farm.id]: weather }));
                    generateFarmSummary(farm.id, weather);
                }).catch(error => console.error(`Error fetching weather for ${farm.name}:`, error));
            }
        });
    })
                ...
                {farms.map((farm) => (
                  <div key={farm.id}>
                    ...
                    {summaries[farm.id]?.length > 0 ? (
                        ...
                        <p>{language === 'fr' ? 'Pas de recommandations disponibles pour l’instant.' : 'لا توجد توصيات متاحة حاليًا.'}</p>
                    )}
                  </div>
                ))}
              </div>
